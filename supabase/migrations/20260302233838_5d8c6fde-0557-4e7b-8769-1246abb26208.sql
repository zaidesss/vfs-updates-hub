...
CREATE OR REPLACE FUNCTION public.get_weekly_scorecard_data(p_week_start date, p_week_end date, p_support_type text DEFAULT 'all'::text)
 RETURNS TABLE(agent_email text, agent_name text, agent_position text, profile_id uuid, quota_email integer, quota_chat integer, quota_phone integer, quota_ot_email integer, day_off text[], mon_schedule text, tue_schedule text, wed_schedule text, thu_schedule text, fri_schedule text, sat_schedule text, sun_schedule text, email_count bigint, chat_count bigint, call_count bigint, ot_email_count bigint, qa_average numeric, revalida_score numeric, days_with_login integer, approved_leave_days integer, planned_leave_days integer, unplanned_outage_days integer, call_aht_seconds integer, chat_aht_seconds integer, chat_frt_seconds integer, order_escalation numeric, productivity_count_override integer, is_saved boolean)
 LANGUAGE plpgsql
...
BEGIN
  ...
  zendesk_metrics AS (
    SELECT 
      ...
      zm.order_escalation,
      zm.productivity_count_override
