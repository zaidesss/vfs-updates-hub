...
BEGIN
  ...
  WITH eligible_agents AS (
    SELECT 
      ...
      ap.full_name,
      ap.position,
      -- Resolve position array to config key
      CASE
        WHEN ap.position @> ARRAY['Email','Chat','Phone'] THEN 'Hybrid'
        WHEN ap.position @> ARRAY['Email','Chat'] THEN 'Email + Chat'
        WHEN ap.position @> ARRAY['Email','Phone'] THEN 'Email + Phone'
        WHEN 'Email' = ANY(ap.position) THEN 'Email'
        WHEN 'Chat' = ANY(ap.position) THEN 'Chat'
        WHEN 'Phone' = ANY(ap.position) THEN 'Phone'
