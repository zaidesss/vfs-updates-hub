...
-- Update the get_weekly_scorecard_data RPC to map positions to only 3 categories: Hybrid, Chat, Logistics
...
BEGIN
  ...
  WITH eligible_agents AS (
    SELECT 
      ...
      ap.full_name,
      ap.position,
      -- Resolve position array to config key (only 3 categories)
      CASE
