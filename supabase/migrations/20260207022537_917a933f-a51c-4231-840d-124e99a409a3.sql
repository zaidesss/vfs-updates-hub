-- Update get_weekly_scorecard_data RPC to return planned_leave_days and unplanned_outage_days separately
-- Also add unplanned_outage_days column to saved_scorecards for persistence

-- First, add the new column to saved_scorecards
ALTER TABLE saved_scorecards 
ADD COLUMN IF NOT EXISTS unplanned_outage_days INTEGER DEFAULT 0;

-- Drop the existing function (both overloads)
DROP FUNCTION IF EXISTS public.get_weekly_scorecard_data(date, date, text);
DROP FUNCTION IF EXISTS public.get_weekly_scorecard_data(text, text, text);

-- Recreate the main RPC function with outage breakdown
