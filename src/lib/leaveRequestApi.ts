...
export interface LeaveRequest {
  ...
  status: 'pending' | 'approved' | 'declined' | 'canceled' | 'pending_override' | 'for_review';
...
// Check for existing Late Login request for the same date
export async function checkExistingLateLoginRequest(
  agentEmail: string,
  date: string
  ...
  try {
    const { data, error } = await supabase
      ...
      .eq('agent_email', agentEmail.toLowerCase())
      .eq('start_date', date)
      .eq('outage_reason', 'Late Login')
      .not('status', 'in', '("declined","canceled")')
      .limit(1);
    
    if (error) {
      console.error('Error checking existing late login request:', error);
      return { data: null, error: error.message };
    }
    
    return { data: data && data.length > 0, error: null };
  } catch (err) {
    console.error('Error checking existing late login request:', err);
    return { data: null, error: 'Failed to check existing request' };
  }
}

// Create auto-generated Late Login outage request
export interface AutoGeneratedRequestInput {
  agent_email: string;
  agent_name: string;
  client_name: string;
  team_lead_name: string;
  role: string;
  start_date: string;
  start_time: string;  // Format: "HH:mm" (e.g., "09:05")
  end_time: string;    // Format: "HH:mm" (e.g., "09:59")
}

export async function createAutoGeneratedLateRequest(
  input: AutoGeneratedRequestInput
): Promise<ApiResponse<LeaveRequest>> {
  try {
    // Calculate duration
    const durations = calculateDurations(
      input.start_date, 
      input.start_date, // Same day for Late Login
      input.start_time, 
      input.end_time
    );
    ...
    const { data, error } = await supabase
      .from('leave_requests')
      .insert({
        agent_email: input.agent_email.toLowerCase(),
        agent_name: input.agent_name,
        client_name: input.client_name,
        team_lead_name: input.team_lead_name,
        role: input.role,
        start_date: input.start_date,
        end_date: input.start_date, // Same day
        start_time: input.start_time,
        end_time: input.end_time,
    ...
    if (error) {
      console.error('Error creating auto-generated request:', error);
      return { data: null, error: error.message };
    }
    
    return { data: data as LeaveRequest, error: null };
  } catch (err) {
    console.error('Error creating auto-generated request:', err);
    return { data: null, error: 'Failed to create auto-generated request' };
  }
}

// Escalated outage request input (admin-triggered from agent reports)
export interface EscalatedOutageInput {
  agent_email: string;
  agent_name: string;
  client_name: string;
  team_lead_name: string;
...
export async function checkExistingOutageRequest(
  agentEmail: string,
  date: string,
  outageReason: string
): Promise<ApiResponse<boolean>> {
  try {
    const { data, error } = await supabase
      ...
      .eq('agent_email', agentEmail.toLowerCase())
      .eq('start_date', date)
      .eq('outage_reason', outageReason)
      .not('status', 'in', '("declined","canceled")')
      .limit(1);
    
    if (error) {
      console.error('Error checking existing outage request:', error);
      return { data: null, error: error.message };
    }
    
    return { data: data && data.length > 0, error: null };
  } catch (err) {
    console.error('Error checking existing outage request:', err);
    return { data: null, error: 'Failed to check existing request' };
  }
}

// Create escalated outage request (admin-triggered from agent reports)
export async function createEscalatedOutageRequest(
  input: EscalatedOutageInput
): Promise<ApiResponse<LeaveRequest>> {
  try {
    // Calculate duration
    const durations = calculateDurations(
      input.start_date, 
      input.start_date, // Same day
      input.start_time, 
      input.end_time
    );
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        outage_reason: input.outage_reason,
        status: 'for_review',
        is_auto_generated: false, // Admin-initiated, not system-triggered
        ...durations
      })
      .select()
      .single();
    ...
    if (error) {
      console.error('Error creating escalated outage request:', error);
      return { data: null, error: error.message };
    }
    
    return { data: data as LeaveRequest, error: null };
  } catch (err) {
    console.error('Error creating escalated outage request:', err);
    return { data: null, error: 'Failed to create escalated outage request' };
  }
}

// Update remarks for auto-generated request
export async function updateAutoGeneratedRemarks(
  id: string,
  remarks: string,
  userEmail: string
): Promise<ApiResponse<LeaveRequest>> {
  try {
    ...
    // Log the change to history
    if (oldData && data) {
      await logLeaveRequestChange(id, userEmail, oldData as LeaveRequest, data as LeaveRequest);
    }
    
    return { data: data as LeaveRequest, error: null };
  } catch (err) {
    console.error('Error updating remarks:', err);
    return { data: null, error: 'Failed to update remarks' };
  }
}
