...
import { fetchEODAnalytics, type EODAnalytics, INCIDENT_TYPE_CONFIG, type IncidentType } from '@/lib/agentReportsApi';
import { cn } from '@/lib/utils';
import { useAuth } from '@/context/AuthContext';

export function EODAnalyticsPanel() {
  ...
  const [analytics, setAnalytics] = useState<EODAnalytics | null>(null);
  ...
  useEffect(() => {
    ...
    const loadAnalytics = async () => {
      ...
      const result = await fetchEODAnalytics(dateStr);
  ...
  const getStatusConfig = (status: EODAnalytics['status']) => {
    switch (status) {
      case 'good':
        return { 
          label: 'Good', 
  ...
  const formatGap = (seconds: number | null | undefined) => {
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <CardTitle className="text-lg">EOD Team Analytics</CardTitle>
              {analytics && (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <Card className="bg-purple-50 dark:bg-purple-950 border-purple-200 dark:border-purple-800">
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Avg Logged</span>
                        <span className="font-medium">{formatHours(analytics.time.avgLogged)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Required</span>
                        <span className="font-medium">{formatHours(analytics.time.avgRequired)}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{analytics.compliance.incidents}</span>
                      </div>
