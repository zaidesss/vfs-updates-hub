...
# Fix: Ticket Logs Dashboard Database Function Type Mismatch

## Root Cause

The `get_ticket_dashboard_data` database function has a **type mismatch** between the declared return type and the actual data type:

| Column | Declared Type (in function) | Actual Type (in table) |
|--------|----------------------------|------------------------|
| `avg_gap_seconds` | `numeric` | `integer` |

This causes PostgreSQL error 42804: "structure of query does not match function result type"

---

## Solution

Update the database function `get_ticket_dashboard_data` to declare `avg_gap_seconds` as `integer` instead of `numeric`.
...
RETURNS TABLE(
  ...
  chat_count bigint,
  call_count bigint,
  avg_gap_seconds integer,  -- Changed from numeric to integer
...
The `ticket_gap_daily` table was created with `avg_gap_seconds` as `integer`, but when the `get_ticket_dashboard_data` function was defined, it mistakenly declared the return type as `numeric`. This worked until PostgreSQL started enforcing strict type checking (likely after a recent migration or database update).

---

## Testing

After the migration, reload the Ticket Logs page at `/team-performance/ticket-logs` to confirm the dashboard data loads correctly.
